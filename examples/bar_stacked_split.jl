...
BokehServer.Plotting.serve() do
    ...
    p = BokehServer.figure(y_range=fruits, height=350, x_range=-16:16, title="Fruit import/export, by year",
               toolbar_location=nothing)

    BokehServer.barstack!(p; x = years, y="fruits", height=0.9, color=GnBu3, source=exports,
        legend_label=["$x exports" for x in years])

    BokehServer.barstack!(p; x = years, y="fruits", height=0.9, color=OrRd3, source=imports,
